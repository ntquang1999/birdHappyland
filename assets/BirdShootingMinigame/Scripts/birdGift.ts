...
export default class BirdGift extends cc.Component {
    ...
    update (dt) {
        ...
        let skt =    this.birdSprite.getComponent(sp.Skeleton);
        if((this.hitpoint == this.maxHitpoint) && !(skt.animation == "fly"))
        {
            skt.animation = "fly";
        } 
        else if((this.hitpoint == (this.maxHitpoint-1)) && !(skt.animation == "fly_2"))
        {
            skt.animation = "fly_2";
        }
        else if((this.hitpoint == 1) && !(skt.animation == "fly_3"))
        {
            //console.log("zzz");
            skt.animation = "fly_3";
        }
    ...
    SelfDestruct(shot: boolean)
    {
        ...
        {
            ShootBirdConfig.birdList.push({"id": this.id, "type": 1, "hitpoint": this.hitpoint, "speed": this.speed, "maxHitpoint": this.maxHitpoint});
            this.node.destroy();
        }     
    }
